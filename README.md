...
一款本地优先的求职投递进度管理网页。把岗位链接、公司、城市和招聘进展放在同一张看板里；收到公司的下一步通知后，选择实际环节，系统会自动补充流程与时间线。

## 功能

- 投递档案：公司、岗位、城市、薪资、岗位链接、标签与备注
...
当前自动化测试覆盖链接识别公司、动态流程、自动推进、跳过环节、跨阶段移动、回退重开、终止状态、跟进日期、统计和备份合并。`scripts/browser-smoke.mjs` 还可连接 Chrome DevTools，执行真实浏览器烟雾测试。
